...
let _pnlConnectionUrl = null;
export function getPnlConnection() {
  const rpcUrl = config.pnl?.rpcUrl || process.env.PNL_RPC_URL || "https://pump.helius-rpc.com";
  if (!_pnlConnection || _pnlConnectionUrl !== rpcUrl) {
    _pnlConnection = new Connection(rpcUrl, "confirmed");
    _pnlConnectionUrl = rpcUrl;
  }
