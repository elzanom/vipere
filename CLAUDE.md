...
- **What it is**: Node 22+ ESM service that runs an LLM-driven loop
  ...
  in the loop. Telegram + Discord provide ops surface; HiveMind provide
  shared learning.
...
| `smart-wallets.json` | 103 | Tracked KOL/alpha wallets. `type: "lp"` (default) checks positions; `type: "holder"` only checks token holdings. 5-min position cache. `check_smart_wallets_on_pool` is the deployment confidence signal. |
...
2. **Indonesian Creator Strategies**: Pre-loaded with custom strategies from Indonesian creators/communities:
   ...
   - `0xyunss_bidask_sideways` — Yunus bid-ask sideways play.
   - `0xyunss_spot_balanced` — Yunus spot balanced play.
3. **Dynamic Strategy Selection**: Automatically swaps the active strategy based on real-time candidate pool average volatility:
   - Average Volatility > 4.00 → Bengshark Bid-Ask Volatility Play.
   - Average Volatility 2.00 - 4.00 → Yunus Bid-Ask Play.
   - Average Volatility < 2.00 / No data → Yunus Spot Balanced Play.
4. **PnL Accounting Sanity Fallback**: Includes a safety Diff threshold check (`pnlSanityMaxDiffPct`, default 5%). If the difference between reported PnL and calculated PnL exceeds this threshold, the agent is warned to avoid trusting the PnL tick to prevent premature close or wrong accounting. Falls back to cached pre-close metrics if closed API fails to settle.
5. **Telegram Ops Hardening**:
   - `sendPhoto` and `generateDailyPnLImage` support — daily morning briefings include a premium visual grid card representation of the portfolio PnL.
   - `force` parameter on `notifyClose` to ensure manual exits are parsed and logged properly without silent dropouts.
   - Dynamic Kelly sizing based on past win rate, lookback days, and wallet balance.
